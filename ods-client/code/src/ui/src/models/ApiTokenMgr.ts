...
import { ApiTokenCreate, ApiTokenInfo, apitokenType } from '../common/common';
...
export class ApiTokenMgr  {
    ...
    async getDocumentForName(name: string): Promise<ApiTokenInfo | null> {
        ...
        if (!name) {
            console.log(`getDocumentForName() error: Document name is missing`);
            throw new Error("ApiTokenMgr.getDocumentForName() - Document name missing");
        }
        let document:ApiTokenInfo;
        try {
            const config = {
