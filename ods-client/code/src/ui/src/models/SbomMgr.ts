...
import { copyDocField } from '../common/util';
//import { Vulnerability } from '../common/bom-1.6';
import { sbomType, SbomDocument, AttachmentInfo, SbomDocumentInfo, SbomDocumentCreate, CommentUpdate, CommentCreate } from '../common/common';
...
const urlPath = `/api/odsServer/docs/${sbomType}/`;
...
export class SbomMgr implements IDocMgr<SbomDocument>, IAttachmentMgr, ICommentMgr<SbomDocument> {
    ...
    async getDocumentForBomRef(bomRef: string): Promise<SbomDocumentInfo | null> {
        ...
        try {
            ...
            // console.log("SbomMgr.getDocumentForBomRef response=", response)
    ...
    async getDocumentsStream(callback: any, body?: any) {
        console.log("SbomMgr.getDocumentsStream() body=",body);
        try {
            await this.http.postStream(urlPath, body, undefined, callback);
        } catch (e: any) {
            const err = new DocError(e);
            console.error(err);
            throw err;
