...
import { ScriptInfo, ScriptSummary, ScriptUpdate, ScriptCreate, scriptType } from '../common/common';
...
export class ScriptMgr  {
    ...
    async runUserScript(data: any): Promise<any> {
        try {
            const response = await this.http.post(`${REACT_APP_ODS_SERVER}/api/script/run`, data);
            console.log("runUserScript response=", response)
            const body = response.data;
            return body;
        } catch (e: any) {
            const err = new DocError(e);
    ...
    async runUserScriptId(id: string, data: any): Promise<any> {
        try {
