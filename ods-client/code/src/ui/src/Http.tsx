...
// This is the Oauth access token received when user authenticated
// It can be used to access the process.env.REACT_APP_ODS_SERVER as the same authenticated user
export class Http {
    baseUrl: string;
    auth: any;
    ...
    private setBearer(config?: AxiosRequestConfig<any>) {
        if (this.accessToken) {
            ...
            try {
                ...
                const response = axios.get(_url + delimit + "stream=compress", {...c, onDownloadProgress: (e: any): void => {
                    try {
                        ...
                        // console.log(">>e=",e, "readyState=", req.readyState);
        ...
        try {
            ...
            const response = await axios.post(_url, {...data, stream:"compress"}, {...c, onDownloadProgress: (e: any): void => {
                try {
                    ...
                    // console.log("CHUNK=",chunk)
                    while (true) {
                        // if this is the first chunk, it'll contain its
                        //  length in the first 16 characters
                        if (len == 0 && chunk.length >= index+16) {
                            const slen = chunk.substring(index, index+16);
                            charsRead += 16;
                            // console.log(">>postStream: slen =",slen);
                            if (slen.startsWith("-")) {
                                done = true;
                                break;
                            }
