...
const MostUsedDataGrid: React.FC<Props> = ({
        ...
        [
...

            {
                    ...
                    if (params && params.data) {
                        ...
                        if (guidance) {
                            if (guidance.state == "active") {
                                let value = [];
                                if (guidance?.pci?.preferredVersion?.minimum) {
                                    value.push("PCI preferred version = "+guidance.pci.preferredVersion.minimum);
                                }
                                if (guidance?.nonPci?.preferredVersion?.minimum) {
                                    value.push("Non-PCI preferred version = "+guidance.nonPci.preferredVersion.minimum);
                                }
                                return (value);
                            }
                ...
                cellRenderer: function (params: any) {
                    // console.log("guidance params.value=",params.value);
                    if (params && params.value) {
                        const found = params.value;
                        let key = 0;
                        const _guidance = <div>
                            {found.map((item: string) => {
                                return (
...

            {
                ...
                headerTooltip: "Deps.dev indicates that latest version of this library was published on this date.",
                cellClass: "center-text",
                valueGetter: function (params: any) {
...

            {
                ...
                cellRenderer: function (params: any) {
                    if (params) {
                        ...
                        return (
                            <ActionPopup>
                                <MenuItem onClick={() => onRowClicked({name: data.name, group: data.group, basePurl: data.basePurl})} >View Software Details</MenuItem>
    ...
    useEffect(() => {
        console.log("MostUsedDataGrid data changed");

        if (data && data.length > 0) {
            console.log("MostUsedDataGrid - looking for additional columns");
...
export default MostUsedDataGrid;
