...
import {CommentInfo, formatDateTime } from '../common';
...
const Comments: React.FC<Props> = ({ document, setDocument, context, topic, customHeading, searchString, docMgr }) => {
    ...
    const [newComment, setNewComment] = useState<EditorState>((ccomment ? EditorState.createWithContent(convertFromRaw(JSON.parse(ccomment))) : EditorState.createEmpty()))
    ...
    async function addComment(event: any): Promise<void> {
        ...
        const r = await docMgr.addComment(document, text, _topic, newCommentPrivate);
        if (r) {
    ...
    const canEditComment = (comment: CommentInfo, context: AppContext) => {
    ...
    async function togglePrivate(index: number, event: any): Promise<void> {
        ...
        const r = await docMgr.updateCommentForId(document, document.comments[index].id, { private: event.target.checked });
        if (r) {
            setDocument(r);
        }
    }
    ...
    async function toggleNewCommentPrivate(event: any): Promise<void> {
        console.log("toggleNewCommentPrivate: ", "checked=", event.target.checked)
    ...
    async function onBlur() {
        window.localStorage.setItem("editorContent-comment", JSON.stringify(convertToRaw(newComment.getCurrentContent())));
    }
    ...
    const showEditor = (index: number, event: any) => {
        ...
        while (container && !container.classList?.contains("commentContainer")) {
    ...
    const updateComment = async () => {
        ...
        if (edit >= 0) {
            ...
            if (document.comments[edit].id) {
                r = await docMgr.updateCommentForId(document, document.comments[edit].id, { text: text });
            }
            if (r) {
                setEdit(-1);
                setDocument(r);
    ...
    const getTooltipTitle = (comment: CommentInfo) => {
        ...
        if (comment.edited) {
            for (var c of comment.edited) {
                r.push(<tr key={"commentEdited-" + c.date}><td>{c.user.name}</td><td>{formatDateTime(c.date)}</td></tr>);
            }
        }
        return (
            <div style={{ padding: "10px" }}>
                <div style={{ paddingBottom: "10px" }}><b>Previous comment edits:</b></div>
    ...
    const showComment = (comment: CommentInfo) => {
        ...
        const isEditable = editMode || comment.user.email == context.user.email
        if (!searchString || searchString == "") {
            if (isPublicComment || isEditable) return true;
        }
        else {
            const textIncludes = comment.text.toLowerCase().includes(searchString.toLowerCase())
            const nameIncludes = comment.user.name.toLowerCase().includes(searchString.toLowerCase())
    ...
    return (
        ...
        {(document.comments.length > 0) && <>
            {document.comments.map((comment: any, i: any) => {
                if (!topic || (topic == "*") || (topic == comment.topic)) {
                    return (
                        <div key={i} className="commentContainer">
                            {(edit != i && showComment(comment)) && <div className="commentDiv">
                                <div style={{ display: "flex", justifyContent: "space-between", gap: "20px" }}>
                                    <div>
                                        ...
                                        {(comment.edited && comment.edited.length > 0) &&
                                            <Tooltip title={getTooltipTitle(comment)}>
                                                <span>&nbsp; (Edited)</span>
                                            </Tooltip>
                                        }
                                        <div>{parse(comment.text)}</div>
                                    </div>
                                    {canEditComment(comment, context) && <div className="commentDeleteButton" style={{ display: "flex", flexDirection: "column" }}>
                                        <Button style={{ marginBottom: "8px" }} onClick={(event) => showEditor(i, event)}>Edit</Button>
                                        <Button onClick={() => deleteComment(comment.id || i)}>Delete</Button>
                                    </div>
                                    }
                                </div>
                            </div>}
        ...
        {(document.comments.length == 0) && <div>
            None
        </div>}

        <div ref={commentEditorContainer}>
            <div ref={commentEditor} style={{ display: (edit >= 0 ? "block" : "none") }} className="commentDiv">
                Edit Comment:
                <div className="editorDiv">
                    <EditorContainer id="comment" data={data} setData={setData} editMode={true} onBlur={onBlurEdit} uploadImageCallback={uploadImageCallback} setError={context.setError} />
        ...
        {canAddComment && <div>
            ...
            New Comment:
            <div className="editorDiv">
                <EditorContainer id="comment" data={newComment} setData={setNewComment} editMode={true} onBlur={onBlur} uploadImageCallback={uploadImageCallback} setError={context.setError} />
